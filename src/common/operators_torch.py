...
def lapl(field, dx, dy, nx, ny, order=2, b=0):
    ...
    if order == 2:
        laplacian[1:-1, 1:-1] = (1 - b) * ((field[2:, 1:-1] + field[:-2, 1:-1] - 2 * field[1:-1, 1:-1]) / dy ** 2 +
                                           (field[1:-1, 2:] + field[1:-1, :-2] - 2 * field[1:-1, 1:-1]) / dx ** 2) + \
                                b * (field[2:, 2:] + field[2:, :-2] + field[:-2, :-2] + field[:-2, 2:] - 4 * field[1:-1,
                                                                                                             1:-1]) \
                                / (2 * dx ** 2)
    elif order == 4:
        laplacian[2:-2, 2:-2] = (- field[4:, 2:-2] + 16 * field[3:-1, 2:-2] - 30 * field[2:-2, 2:-2] + 16 * field[1:-3,
                                                                                                            2:-2]
                                 - field[:-4, 2:-2]) / (12 * dy ** 2) + \
                                (- field[2:-2, 4:] + 16 * field[2:-2, 3:-1] - 30 * field[2:-2, 2:-2] + 16 * field[2:-2,
                                                                                                            1:-3]
                                 - field[2:-2, :-4]) / (12 * dx ** 2)
